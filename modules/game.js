...
export function executeGame() {
	...
	if (sessionStorage.mode == "story") {
		...
		let field = document.getElementById("game"),
			...
			columns = Number(sessionStorage.bricksNumberColumn),
			rows = Number(sessionStorage.bricksNumberRow);
		...
		parameters = {
			brick: [(field.offsetHeight - 20 - gap * (columns - 1)) / columns, (field.offsetWidth * 0.4 - gap * (rows - 1)) / rows],
		...
		console.log(parameters.brick);

		environmentColors = {
			field: sessionStorage.fieldColor,
	...
	let score = 0,
		...
		brickHeight = parameters["brick"][0],
		brickWidth = parameters["brick"][1],
		paddleWidth,
		paddleHeight,
		bricks = [],
		sec = 0,
		min = 0,
		t,
		orientation = sessionStorage.orientation;
		...
		for (let row = 0; row < arr.length; row++) {
			for (let col = 0; col < arr[row].length; col++) {
				...
				if (arr[row][col] != 0) {
					...
					bricks.push({
						x:
							option == "horizontal" ? wallSize + (brickWidth + brickGap) * row + field.offsetWidth * 0.05 : wallSize + (brickWidth + brickGap) * col,
						y:
							option == "horizontal"
								? wallSize + (brickHeight + brickGap) * col
								: wallSize + (brickHeight + brickGap) * row + field.offsetHeight * 0.05,
						color: colorMap[colorCode],
						width: brickWidth,
						height: brickHeight,
					});
					console.log("y:");
					console.log(wallSize + (brickHeight + brickGap) * col);
				}
			}
		}
	}
	orientation == "horizontal" ? bricksFill(level_bricks, "horizontal") : bricksFill(level_bricks, "vartical");

	const paddle = {
		x: orientation == "horizontal" ? field.offsetWidth * 0.94 : field.offsetWidth / 2 - paddleWidth / 2,
		y: orientation == "horizontal" ? field.offsetHeight / 2 - paddleWidth / 2 : field.offsetHeight * 0.9,
		width: paddleWidth,
		height: paddleHeight,
	...
	const ball = {
		x: orientation == "horizontal" ? field.offsetWidth * 0.6 : field.offsetWidth * 0.4,
		y: orientation == "horizontal" ? field.offsetHeight * 0.4 : field.offsetHeight * 0.6,
	...
	function loop() {
		...
		//console.log(`paddle.dx: ${paddle.dx}; paddle.dy ${paddle.dy}`);

		requestAnimationFrame(loop);
		context.clearRect(0, 0, field.offsetWidth, field.offsetHeight);
		orientation == "horizontal" ? (paddle.y += paddle.dy) : (paddle.x += paddle.dx); // движение платформы с заданной скоростью
	...
	field.addEventListener("touchmove", (e) => {
		...
		if (radioTouchscreen.checked) {
			orientation == "horizontal" ? (paddle.y = touchPosition.y - paddleHeight / 2) : (paddle.x = touchPosition.x - paddleWidth / 2);
		}
	});

	// нажатие клавиши
	document.addEventListener("keydown", function (e) {
		if (radioKeyboard.checked && orientation == "horizontal") {
		...
		} else if (radioKeyboard.checked && orientation == "vertical") {
	...
	document.addEventListener("keyup", function (e) {
		if (radioKeyboard.checked && orientation == "horizontal" && (e.which === 87 || e.which === 83 || e.which === 38 || e.which === 40)) {
			paddle.dy = 0;
		} else if (radioKeyboard.checked && orientation == "vertical" && (e.which === 65 || e.which === 37 || e.which === 68 || e.which === 39)) {
			paddle.dx = 0;
		}
	});
	...
	document.addEventListener("mousemove", (e) => {
		...
		if (radioMouse.checked && orientation == "horizontal") {
			paddle.y = pointerPosition.y - paddleHeight;
		} else if (radioMouse.checked && orientation == "vertical") {
